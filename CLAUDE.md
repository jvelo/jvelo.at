...
- **Layout**: one centered column. `--page-width` (56rem) bounds the page, `--measure` (38rem) bounds prose, `--gutter` is the side padding, `--section-gap` separates home sections. Images with `.border-backdrop` get a `--backdrop-offset` inset so the offset backdrop aligns with the text edge
